...
export class DialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.editData) {
      ...
      this.productForm.controls['eventDescription'].setValue(
        this.editData.eventDescription
      );
      this.productForm.controls['venue'].setValue(this.editData.venue);
      this.productForm.controls['ticketPrice'].setValue(
        this.editData.ticketPrice
      );
      this.productForm.controls['eventDuration'].setValue(
        this.editData.eventDuration
      );
  ...
  addEvents() {
    if (!this.editData) {
      ...
      if (this.productForm.valid) {
        console.log('add2');
        this.api.postEvent(this.productForm.value).subscribe({
          next: (res) => {
            console.log('add3');
          ...
          error: () => {
            alert('Error while adding the eventt');
          },
        });
      } else {
        console.log('add4');
  ...
  onUpload() {
    ...
    // this.http
    //   .post('http://localhost:3000/posts/', this.selectedFiles)
    //   .subscribe((res) => {
    //     console.log(res);
    //     });
  }
